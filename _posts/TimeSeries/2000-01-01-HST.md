...
and recently it was published also in **[Applied Intelligence](https://link.springer.com/article/10.1007/s10489-021-02897-z)**
journal.
...
and a fast algorithm to find them: HOT SAX).
...
but I used them during my researches). This version of the code has also other minor bugs.
...
#  Compiling the Code
The present code is written in Fortran. Be aware that it contains a lot of subroutines no longer in use, it should be
...
You can find a list of links for compilers at the **[Fortran webpage](https://fortran-lang.org/)** (including the Gfortran).
...
# The input file 
At this you need to configure the discord search with the **input.dat** file:

```console
25        eXl,         elementi for each letter
...
HST allows you to search for the discord of length *m* within the time series.
...
m = eXl x lung_parole
```
This is a limitation, since you cannot search for sequences that have a length that is a prime number
since it has to be the product of two integers (>2).
For example, if you need m =300 you can pick:
```console
eXl = 75
lung_parole = 4
```
...
**Znorm** tells the computer to calculate the Z-normalized Euclidean distance (if T ) or the regular Euclidean distance F

The last parameter to be changed is the algorithm name. The name of the HST algorithm in the code is 'altro' 
(it means other in Italian, it was a derivation of HOT SAX, and I never changed it) 
...
in a txt like file, as for example the file **chfdbchf15_1.csv** begins in this way:
```console
-0.215
-0.24
-0.24
...
By using the command *time* at the end of the execution you will get information regarding the execution time. 
...
Take into account that most codes count from 0, while for me the first sequence is at position 1, so there might
be 1 point difference between my results and those of other discord codes.

Example of a run (sorry the explanation is in Italian):
